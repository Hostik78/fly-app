...
Дата: 2026-07-28, обновлено 2026-07-28
Статус: согласовано с пользователем, реализуется

**Обновление:** первая версия делала все поля обязательными (кнопка
«Продолжить» была заблокирована, пока не заполнено всё). Пользователь явно
попросил сделать их необязательными — можно нажать «Продолжить», ничего не
указав. Ниже описание уже с этим учётом: `gender`/`age`/`height`/`languages`
могут быть `null`, колонки в базе без `not null` (check-ограничения на
диапазон при этом не мешают — в Postgres они автоматически считаются
выполненными для `NULL`). Также возраст/рост/языки в итоге сделаны не
текстовыми полями, а пикерами (выпадающие списки для возраста и роста;
разворачивающийся список с поиском для языков, построенный на полном списке
кодов ISO 639-1 и `Intl.DisplayNames` для русских названий) — сам текст ниже
изначально описывал текстовые поля, актуальная реализация в
`ProfileSetupScreen.tsx` и `src/data/languages.ts`.
