...
//
// onReady (см. App.tsx) - вызывается, как только видео РЕАЛЬНО начало
// показывать кадры (событие 'playing'), а не по истечении заранее выбранного
// времени - переход к остальному приложению происходит сразу, как только это
// произошло И данные (профиль, гео и т.п.) готовы, что раньше наступит. Если
// видео по какой-то причине (например, нет сети) так и не смогло начать
// играть - запасной таймер на 2.5 секунды всё равно даёт команду двигаться
// дальше, чтобы экран загрузки не завис бесконечно из-за одного лишь ролика.
...
export function LoadingScreen({ onReady }: LoadingScreenProps) {
  ...
  // Общий "флажок" между запасным таймером и событием 'playing' - неважно,
